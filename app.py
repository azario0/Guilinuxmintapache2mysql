...
class ServerControlApp:
    def __init__(self, root, sudo_password):
        ...
        # Disable on Startup
        tk.Checkbutton(root, text="Disable MySQL on Startup", variable=self.mysql_disable_startup, command=self.toggle_mysql_startup).grid(row=2, column=0, columnspan=2, sticky="w")
        tk.Checkbutton(root, text="Disable Apache on Startup", variable=self.apache_disable_startup, command=self.toggle_apache_startup).grid(row=3, column=0, columnspan=2, sticky="w")

        # Check Status Button
        tk.Button(root, text="Check Status", command=self.check_status).grid(row=4, column=0, columnspan=4)
